...
sim <- function(seed){
  ...
  dataset <- data[[1]]
  weights <- data[[2]]
  dataset <- cbind(LDL = data_chol,dataset)
  
  load("data_test_LDL_adj_ApoB.RData")
  train = dataset
  test = test 
  test$LDL <- test$LDL_adj
  weights_train=weights
  
  rm(data,dataset,data_chol) # removed the fist fitted model
  gc(reset=T)
  
  set.seed(seed)
  
  mod = gamboostLSS(cbind(LDL,ApoB)~., data = train, baselearner = "bols", control = boost_control(mstop = 5000, nu = 0.01, risk = "oobag", trace = T), 
                    weights = weights_train, method = 'noncyclic', families =  Gauss_Cop(marg1 = "LOGLOG", marg2 = "GA", stabilization = "MAD"))
  ...
  mod = gamboostLSS(cbind(LDL,ApoB)~., baselearner ="bols", data = train, control = boost_control(mstop = MSTOP, nu = 0.01, trace = T),
                    method = 'noncyclic', families = Gauss_Cop(marg1 = "LOGLOG", marg2 = "GA", stabilization = "MAD"))
  ...
  LogLik_values <- loss(y = test[,c('LDL','ApoB')], mu1 = pred_mu1, mu2 = pred_mu2,
                        sigma1 = pred_sigma1, sigma2 = pred_sigma2, rho = pred_rho)
  LogLik <- sum(loss(y = test[,c('LDL','ApoB')], mu1 = pred_mu1, mu2 = pred_mu2,
  ...
  save(res, file = paste0("sims_LDL_apob_classic_MAD_",seed,".RData"))
  ...
  mod_desel <- DeselectBoostLSS_5(mod, tau = tau_1, fam = Gauss_Cop(marg1 = "LOGLOG", marg2 = "GA"))
  ...
  LogLik_desel <- sum(loss(y = test[,c('LDL','ApoB')], mu1 = pred_mu1_desel, mu2 = pred_mu2_desel,
  ...
  save(res, file = paste0("sims_ldl_apob_desel_MAD_",seed,".RData"))
  

  return(list(Coefficients=Coefficients, Coefficients_desel = Coefficients_desel, mstop=mstop.mod, pred=pred, pred_desel=pred_desel, LogLik_values=LogLik_values, LogLik=LogLik, LogLik_desel = LogLik_desel))
...
save(results, file="LDL_ApoB_models_opt_MAD.RData")
