###########################################################
###########################################################
# ----- Simulation Clayton - Modification of Hans et al-- #
...
dat <- function(data,job){
  ...
  for(i in 1:(n_train + n_eval)){
    data_run <- data.gen(FAM = "clayton", s1 = s1, s2 = s2, s3 = s3, s4 = s4, Z = X_train, i = i)
  ...
  for(i in 1:n_test){
    data_run <- data.gen(FAM = "clayton", s1 = s1, s2 = s2, s3 = s3, s4 = s4, Z = X_test, i = i)
...
classic <- function(job,data,instance){
  ...
  time = Sys.time()
  mod = gamboostLSS(form, data = train, control = boost_control(mstop = 1000, nu = 0.01, risk = "oobag"), weights = weights_train, method = 'noncyclic', families = Clayton_Cop(marg1 = "LOGNO",
                                                                                                                                                                                marg2 = "LOGLOG"))
  ...
  mod = gamboostLSS(form, data = train, control = boost_control(mstop = MSTOP, nu = 0.01), method = 'noncyclic', families = Clayton_Cop(marg1 = "LOGNO",marg2 = "LOGLOG"))
...
probing <- function(job, data, instance){
  ...
  mod = gamboostLSS(cbind(y1,y2) ~., data = probes.train, control = boost_control(mstop = 2000, nu=0.01),
                    method = 'noncyclic', families = Clayton_Cop(marg1 = "LOGNO",
                                                                 marg2 = "LOGLOG"))
...
stability <- function(job, data, instance,q){
  ...
  pred <- vector('list')
  LogLik <- vector('list')
  energy_score <- vector('list')
  Coefficients_all <- vector('list')
  
  for(i in 1:length(pfer)){
    
    if(i == 1){
      s1 <- stabsel(mod, q =  q, PFER = pfer[1])
    ...
    mod = gamboostLSS(form, data = train,
                      control = boost_control(mstop = 5000, nu = 0.01,risk = 'oobag'), weights = weights_train, method = 'noncyclic', families = Clayton_Cop(marg1 = "LOGNO",
                                                                                                                                                             marg2 = "LOGLOG"))
    ...
    rm(mod) # removed the fist fitted model
    mod = gamboostLSS(form, data = train1, control = boost_control(mstop = MSTOP, nu = 0.01), method = 'noncyclic', families = Clayton_Cop(marg1 = "LOGNO",marg2 = "LOGLOG"))
    ...
    energy_score[[i]] <- mean(es_copula)
  }

  list(instance, Coefficients=Coefficients_all, TP=TP, FP=FP,  mstop=mstop.mod, pred=pred, LogLik=LogLik, es_copula = energy_score, values = list(n=n, p=p, q=q, pfer=pfer), runtime = runtime_all)
}
...
deselection_change <- function(job, data, instance,tau_1){
  ...
  time = Sys.time()
  mod = gamboostLSS(form, data = train, control = boost_control(mstop = 5000, nu = 0.01, risk = "oobag"), weights = weights_train, method = 'noncyclic', families = Clayton_Cop(marg1 = "LOGNO",
                                                                                                                                                                                marg2 = "LOGLOG"))
  MSTOP <- which.min(risk(mod,merge = T))
  oobag.risk <- risk(mod,merge = T)
  ...
  mod_desel <- DeselectBoostLSS_5(mod, tau = tau_1, fam = Clayton_Cop(marg1 = "LOGNO",
                                                                      marg2 = "LOGLOG"))
