###########################################################
###########################################################
# ------ Simulation Gumbel - Setting Hans et al. -------- #
...
dat <- function(data,job){
  ...
  TrueBeta$sigma1 <- c(0,0,1,rep(0,times = p-3))
...
stability <- function(job, data, instance,q){
  ...
  mod = gamboostLSS(form, data = train1,
                    control = boost_control(mstop = 5000, nu = 0.01),method = 'noncyclic', families = Gumbel_Cop(marg1 = "LOGNO",
                                                                                                                          marg2 = "LOGLOG"))
...
deselection <- function(job, data, instance,tau_1){
  ...
  DeselectBoostLSS_5 <- function(object, data = NULL, fam, tau = NULL, method = c('attributable','cumulative')){
    ...
    if(inherits(object,'glmboostLSS')){
      if(any(select[[1]] ==1) ) select[[1]] <- select[[1]]-1
      if(any(select[[2]] ==1) ) select[[2]] <- select[[2]]-1
      if(any(select[[3]] ==1) ) select[[3]] <- select[[3]]-1
      if(any(select[[4]] ==1) ) select[[4]] <- select[[4]]-1
      if(any(select[[5]] ==1) ) select[[5]] <- select[[5]]-1
    ...
    if( select[[5]] == 0) w.parameter <- w.parameter[!w.parameter==parameter[5]]
    Risk.order <- data.frame(w.parameter, n.parameter, unlist(Risk.Var)[!is.na(unlist(Risk.Var))])
    ...
    if(any(Risk.order$parameter == parameter[1])){
      Risk <- Risk.order[Risk.order$parameter == parameter[1],]
      para1 <- Risk[Risk$Risk > sum(Risk[,'Risk'])*tau,]
      if(!empty(para1)){
        form1 <- as.formula(paste(name.response, " ~ ", paste(para1$VarName, collapse= "+")))
      }else{  form1 <-  as.formula(paste(name.response, '~bols(Intercept, intercept=F)'))}
    }
    
    if(any(Risk.order$parameter == parameter[2])){
      Risk <- Risk.order[Risk.order$parameter == parameter[2],]
      para1 <- Risk[Risk$Risk > sum(Risk[,'Risk'])*tau,]
      if(!empty(para1)){
        form2 <- as.formula(paste(name.response, " ~ ", paste(para1$VarName, collapse= "+")))
      }else{  form2 <-  as.formula(paste(name.response, '~bols(Intercept, intercept=F)'))}
    }
    
    if(any(Risk.order$parameter == parameter[3])){
      Risk <- Risk.order[Risk.order$parameter == parameter[3],]
      para1 <- Risk[Risk$Risk > sum(Risk[,'Risk'])*tau,]
      if(!empty(para1)){
        form3 <- as.formula(paste(name.response, " ~ ", paste(para1$VarName, collapse= "+")))
      }else{  form3 <-  as.formula(paste(name.response, '~bols(Intercept, intercept=F)'))}
    }
    
    if(any(Risk.order$parameter == parameter[4])){
      Risk <- Risk.order[Risk.order$parameter == parameter[4],]
      para1 <- Risk[Risk$Risk > sum(Risk[,'Risk'])*tau,]
      if(!empty(para1)){
        form4 <- as.formula(paste(name.response, " ~ ", paste(para1$VarName, collapse= "+")))
      }else{  form4 <-  as.formula(paste(name.response, '~bols(Intercept, intercept=F)'))}
    }
    
    if(any(Risk.order$parameter == parameter[5])){
      Risk <- Risk.order[Risk.order$parameter == parameter[5],]
      para1 <- Risk[Risk$Risk > sum(Risk[,'Risk'])*tau,]
      if(!empty(para1)){
        form5 <- as.formula(paste(name.response, " ~ ", paste(para1$VarName, collapse= "+")))
  ...
  mod_desel <- DeselectBoost(mod, fam = Gumbel_Cop(marg1 = "LOGNO",
                                                   marg2 = "LOGLOG"), tau = tau_1)
