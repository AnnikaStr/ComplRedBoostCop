###########################################################
###########################################################
# ------- Simulation Gauss - Setting Hans et al. -------- #
...
dat <- function(data,job){
  ...
  for(i in 1:n_test){
    data_run <- data.gen(FAM = "normal", s1 = s1, s2 = s2, s3 = s3, s4 = s4, Z = X_test, i = i)
...
classic <- function(job,data,instance){
  ...
  time = Sys.time()
  mod = gamboostLSS(form, data = train, control = boost_control(mstop = 5000, nu = 0.01, risk = "oobag"), weights = weights_train, method = 'noncyclic', families = Gauss_Cop(marg1 = "LOGNO",
                                                                                                                                                                              marg2 = "LOGLOG"))
  ...
  mod = gamboostLSS(form, data = train, control = boost_control(mstop = MSTOP, nu = 0.01), method = 'noncyclic', families = Gauss_Cop(marg1 = "LOGNO",marg2 = "LOGLOG"))
...
deselection_change <- function(job, data, instance,tau_1){
  ...
  mod_desel <- DeselectBoostLSS_5(mod, tau = tau_1, fam = Gauss_Cop(marg1 = "LOGNO",
                                                                    marg2 = "LOGLOG"))
