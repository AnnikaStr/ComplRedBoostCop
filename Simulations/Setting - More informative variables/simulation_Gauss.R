################################################################
################################################################
# ---- Simulation Gauss ---- linear setting, 10 inf. var ----- #
...
source("Gauss_Copula.R")


reg <- makeExperimentRegistry("VariableSelection_Gauss", seed = 123L, packages = c("gamboostLSS","copula"))
...
dat <- function(data,job){
  ...
  for(i in 1:n_test){
    data_run <- data.gen(FAM = "normal", Z = X_test, i = i)
...
classic <- function(job,data,instance){
  ...
  time = Sys.time()
  mod = glmboostLSS(form, data = train, control = boost_control(mstop = 5000, nu = 0.01, risk = "oobag",trace=T), weights = weights_train, method = 'noncyclic', families = Gauss_Cop(marg1 = "NO",
                                                                                                                                                                                      marg2 = "NO"))
  ...
  mod = glmboostLSS(form, data = train, control = boost_control(mstop = MSTOP, nu = 0.01), method = 'noncyclic', families = Gauss_Cop(marg1 = "NO",marg2 = "NO"))
  ...
  loss <- function (y, mu1, sigma1, mu2, sigma2, rho){
    -(log(dnorm(x = y[, 2], mean = mu2, sd = exp(sigma2))) + log(dnorm(x = y[,  
                                                                             1], mean = mu1, sd = sigma1)) - 0.5 * log(1 - rho^2) + (rho/(1 -  
                                                                                                                                            rho^2)) * qnorm(pnorm(q = y[, 2], mean = mu2, sd = exp(sigma2))) *  
        qnorm(pnorm(q = y[, 1], mean = mu1, sd = sigma1)) - (rho^2/(2 *  
                                                                      (1 - rho^2))) * (qnorm(pnorm(q = y[, 2], mean = mu2, sd = exp(sigma2)))^2 +  
                                                                                         qnorm(pnorm(q = y[, 1], mean = mu1, sd = sigma1))^2)) 
  ...
  for(pred_sim_run in 1:1000){
    ...
    Cop <- ellipCopula(family = "normal", dim = 2, param = pred_rho[pred_sim_run] , df = 4)
...
probing <- function(job, data, instance){
  ...
  mod = glmboostLSS(cbind(y1,y2) ~., data = probes.train, control = boost_control(mstop = 2000, nu=0.01),
                    method = 'noncyclic', families = Gauss_Cop(marg1 = "NO",
                                                               marg2 = "NO"))
...
stability <- function(job, data, instance,q){
  ...
  for(i in 1:length(pfer)){
    ...
    mod = glmboostLSS(form, data = train,
                      control = boost_control(mstop = 5000, nu = 0.01,risk = 'oobag'), weights = weights_train, method = 'noncyclic', families = Gauss_Cop(marg1 = "NO",
                                                                                                                                                           marg2 = "NO"))
    ...
    mod = glmboostLSS(form, data = train1, control = boost_control(mstop = MSTOP, nu = 0.01), method = 'noncyclic', families = Gauss_Cop(marg1 = "NO",marg2 = "NO"))
...
deselection_change <- function(job, data, instance,tau_1){
  ...
  time = Sys.time()
  mod = glmboostLSS(form, data = train, control = boost_control(mstop = 5000, nu = 0.01, risk = "oobag"), weights = weights_train, method = 'noncyclic', families = Gauss_Cop(marg1 = "NO",
                                                                                                                                                                              marg2 = "NO"))
  MSTOP <- which.min(risk(mod,merge = T))
  ...
  mod_desel <- DeselectBoostLSS_5(mod, tau = tau_1, fam = Gauss_Cop(marg1 = "NO",
                                                                    marg2 = "NO"))
